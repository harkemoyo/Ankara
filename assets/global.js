...
class LocalizationForm extends HTMLElement {
  constructor() {
    ...
    this.formWrapper = this.querySelector(this.selectors.formWrapper);
    this.formLocalizationInputs = this.querySelector(
      this.selectors.formLocalizationInputs
    );
    this.button = this.querySelector(this.selectors.button);
    this.localizationSpan = this.querySelector(this.selectors.localizationSpan);

    // Adding event listeners
    this.addEventListener("click", (ev) => this.handleSelectDropdownClick(ev));
  }

  // Listen for clicks on the dropdown
  handleSelectDropdownClick(ev) {
    let clickedElement = ev.target;
    // Check if the clicked element is a localization tag
    if (clickedElement.classList.contains(this.classes.localizationTags)) {
      this.changeLanguage(clickedElement.dataset.value);
    }
  }
  // Function to
  changeLanguage(selectedValue) {
    this.localizationSpan.textContent = selectedValue;
    this.formLocalizationInputs.value = selectedValue;
    if (selectedValue) this.formWrapper.submit();
  }
}
...
function renderContactCluster() {
  const waNumber = window.STORE_CONFIG?.WHATSAPP_NUMBER || window.WHATSAPP_NUMBER;
  if (!waNumber) return;
  const callNumber = window.STORE_CONFIG?.PHONE_NUMBER || waNumber;

  const pageUrl = window.location.href;
  const pageTitle = document.title || '';
  const message = `Hi, I have a question about ${pageTitle} (${pageUrl}).`;
  ...
  const chatIcon = `<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>`;
  const closeIcon = `<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>`;

  const container = document.createElement('div');
  container.className = 'floating-contact-cluster';
  container.setAttribute('aria-label', 'Contact options');

  const menu = document.createElement('div');
  menu.className = 'floating-contact-menu';
  menu.innerHTML = `
    <a href="tel:+${callNumber.replace(/\D/g, '')}" class="floating-contact-btn floating-contact-call" aria-label="Call us" title="Call us">${phoneIcon}</a>
    <a href="https://wa.me/${waNumber.replace(/\D/g, '')}?text=${encodeURIComponent(message)}" target="_blank" class="floating-contact-btn floating-contact-whatsapp" aria-label="Chat on WhatsApp" title="Chat on WhatsApp">${whatsappIcon}</a>
  `;
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", renderContactCluster);
} else {
