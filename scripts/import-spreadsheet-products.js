// scripts/import-spreadsheet-products.js
// ============================================================================
// Import products from spreadsheet data to Supabase
// Handles variants, sizes, descriptions, and image mapping
// ============================================================================

require('dotenv').config();
const { supabaseAdmin } = require('../src/config/supabase');

// Spreadsheet data from user's inventory system
...
// Helper function to normalize size names
...
async function importProducts() {
  console.log('🚀 Starting product import from spreadsheet data...\n');

  for (const productData of spreadsheetProducts) {
    try {
      console.log(`📦 Processing: ${productData.title} (${productData.product_id})`);

      // Generate handle from title
      const handle = generateHandle(productData.title);

      // Extract unique sizes from variants
      const uniqueSizes = [...new Set(productData.variants.map(v => normalizeSize(v.size)))];
      ...
      const productPayload = {
        ...
        tags: [productData.product_type.toLowerCase(), productData.collection, 'new'],
        sizes: uniqueSizes,
        in_stock: true,
        // Add features to description for now (could be metafields later)
        seo_title: productData.title,
        seo_description: productData.description.substring(0, 160)
      };
      ...
      console.log(`✅ Product created/updated: ${product.title} (ID: ${product.id})`);

      // Create variants if table exists
      try {
        ...
        for (const variant of productData.variants) {
          const variantTitle = variant.size;
          const variantPrice = variant.price || productData.price;
          const normalizedSize = normalizeSize(variant.size);
          
          const variantPayload = {
            product_id: product.id,
            title: variantTitle,
            sku: `${productData.product_id}-${normalizedSize}`,
            price: variantPrice,
          ...
          if (variantError) {
            console.warn(`⚠️ Warning: Could not create variant ${variantTitle}:`, variantError.message);
          } else {
            console.log(`   ✓ Variant created: ${variantTitle} (SKU: ${variantPayload.sku})`);
          }
        }
      } catch (variantErr) {
        console.warn(`⚠️ Variants table may not exist: ${variantErr.message}`);
      }

      // Link to collections if table exists
      try {
        const { data: collections } = await supabaseAdmin
          .from('collections')
          .select('id')
          .eq('handle', productData.collection);
        ...
        if (collections && collections.length > 0) {
          ...
          console.log(`   ✓ Linked to collection: ${productData.collection}`);
        }
      } catch (collectionErr) {
        console.warn(`⚠️ Could not link to collection: ${collectionErr.message}`);
      }
